...
=head2 Introducing Test::CSV::Pivot
...
to the solution, lets understand the key parameters to the constructor.

=over 2

=item input_file

This would have the input CSV file (full path). This is mandatory parameter.

=item col_key_idx

The column index in the input CSV file that you want to be the unique in the
result CSV file. The index always starts with 0 (zero). This is mandatory parameter.

=item col_name_idx

The column index in the input CSV file that you want to become the column in the
result CSV file. This is mandatory parameter.

=item col_value_idx

The column index in the input CSV file that would provide the value for the
corresponding column with C<col_name_idx>. This is mandatory parameter.
...
Now lets try to get the result as mentioned above. Lets call the input CSV
...
The distribution comes with a tool C<csv-pivot> that can be used out of the box with
...
Lets try one more example, this time we don't want the column "Year" in the result
...
If you any suggestions to improve or found bugs in the code then please raise it on L<GitHub|https://github.com/manwar/Text-CSV-Pivot>.
