...
written in the old and outdated dialet of JavaScript.
...
(Babble itself along with its non-core dependancies is written in pure Perl
...
programatically alter the source code as perl compiles.  What makes Babble so
different?
...
the same source;  A Babble plugin works by changing the way the grammer is
...
transpile the source code on your local machine and then ship that transfomed
...
Bring support for postfix derefencing (C<< ->*% >> and C<< ->*@ >>) to older
versions of perl.
...
Bring support for subsituion regular expressions returning the changes rather
