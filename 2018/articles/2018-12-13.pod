...
The Nightmare begins for our mainainr - this function needs yet more changes.
He needs to add a C<delivery_time> argument to account for the time of day
when presents are delivered (which again, aparently, varies between country).
...
The hacky solution that maintainer can implment is to put a new argument after last mandatory field.
Pity our poor maintainer who chooses this solution and failed to update every
last place that the function is called in the production code and accidentially calls the function
...
points are required for such a compilcated plan, multiplied by number of modules that uses this function.
...
Reducing the number of necessary changes into mearly:
