...
=head2 Method::ParamValidator - Yet another validator for method parameters.

Why another? I was looking for param validators that can be shared among
various Google API interfaces that I have worked on. None of the existing
solutions worked for me. So I decided to create simple, yet easily configurable,
param validator. The best part of this solution is that you can add the param
validation programmatically as well as configuration file.

=head2 DESIGN

There are two core methods C<add_field()> and C<add_method()> that can be used
to setup validator of type L<Method::ParamValidator>.

=head2 add_field(\%params)

First step in creating a validator is to add fields that can be shared by one or
many methods in a validator. You can provide the following keys to add new
field.
...
<tr>
<td>format</td>
<td>data type of the field (optional), possible values are 's' and 'd'. Default is 's'.</td>
</tr>
<tr>
<td>check</td>
...
=for :html
<table border="1">
<tr><td>name</td><td>unique method name (required).</td></td>
<tr><td>fields</td><td>hashref with field names.</td></tr>
</table>

=head2 Example

Suppose, we want to validate method C<add_user()> and it accepts parameters as
hashref with required keys C<firstname>, C<lastname> and C<age>. There is an
optional key C<sex> as well. We will first add all the fields first and then we
will add the method.
...
We can setup the validator using configuration file C<config.json>. Here is the
setup to replicate the above checks.
...
=head2 Check Method Hookup

When adding field to a validator, you can hookup your own checks. Below we are
adding new field C<location> with custom check method C<lookup()>.
...
If you want to plug the validator to an existing C<Moo> package. Lets create
