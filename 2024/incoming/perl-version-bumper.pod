...
Every year, sometime around the end of May, the Perl 5 Porters gift us
with a new version of Perl. Or is it C<perl>?
...
enabled. This leaves time to the code's author to look at their code
...
=head2 The gift of choice

The compromise that preserves backwards-compatibility is that people
have to I<opt-in> the new features. The unfortunate side-effect is that
Perl will continue to look the same (and give the feeling it's stagnating)
if you never enable the new features!
...
in previous version of Perl can be introduced without a guard, and
many were. The L<Syntax::Construct> module has an exhaustive list of
"syntactic constructs that are not implemented via the feature pragma,
but are still not compatible with older versions of Perl".

Perl v5.40 knows about 25 features. While they could be enabled or disabled
(some features are used to disable undesirable constructs, like indirect
object notation) one by one, there is a better way.
...
=head2 Enforce running with a minimum version of the perl binary
...
demand a version that is larger than that of the perl binary running
...
=head2 Load the corresponding feature bundle

The C<feature> module also defines "feature bundles", which allow to
load multiple features together. (See L<feature/FEATURE-BUNDLES> for
their definitions.)
...
In addition to the implicit loading of features, C<use v5.12;> or
greater will enable C<strict>, C<use v5.36;> (actually v5.35) will
enable C<warnings> and C<use v5.40;> (actually, v5.39) will import
L<builtin> functions (using a similar version bundle scheme: see
L<builtin/Version-Bundles>).
...
The features included in bundles fall in two broad categories:
new features, and deprecated features. Loading a bundle will perform
the following two operations in a single line of code:

=head3 Enable modern Perl features
...
official when they are included in a feature bundle. Failed experiment
can be removed.
...
declaration (C<use v5.36>, or any later version) allows to write Perl
subroutines using signatures.
...
=head3 Deprecate discouraged features
...
bundle is implicitely loaded (it only contains features that are
...
Perl script of module I<declares> which version of the I<Perl language>
...
Since Perl v5.36, these two statements are implicitely included inr
...
deprecated feature (the latter is really not recommended), to fine-tune
the specific dialect of Perl that the code is written in.

Future version of C<perl> implicitely promise that they will understand
...
updated, and so is the Perl language. The Perl 5 Porters support
the two most recent stable releases of Perl, which makes a good reason
to upgrade your binary. You don't have to upgrade the version of
the language your code uses, though.
...
to get a taste of those new features on version of C<perl>
...
In 2024, the main Linux distributions all include a version of Perl that
...
different constraints than an Open Source project or library code
distributed on CPAN.
...
latest version of the language very quickly, though:
...
=head2 Updating your Perl code at your own pace

At work, across all of our tens of thousands of Perl modules,
over 30 different C<VERSION> are declared with C<use VERSION>. From
v5.1 (someone typoed 5.010 as 5.001) up to v5.36, going through v5.10,
v5.18.2 (someone didn't know the sub-version is ignored), v5.24, etc. Many
files don't have a C<use VERSION> line (using the Perl flavor of 2002,
when our Perl code base started).  The word for this is "legacy".
...
=head2 Updating your Perl code faster

With tens of thousands of files to potentially update, I didn't imagine
a second to do it manually. I also didn't want to run the code.
So I wrote a module that would take Perl code, and bump the declared
version to the requested one, being extra careful about the issue
detailed above. Since that code contained no company secret, I was
allowed to open source it. It now lives on CPAN as L<Perl::Version::Bumper>.

TODO: insert code examples

Since the module is meant to upgrade older Perl code, I made sure it
can run on C<perl> binaries as old as v5.10. And it can use perl5.10.0
