...
=head1 Using Valiant for Validations in Agendum: A Deep Dive into DBIx::Class Integration
...
which I hope has enough interesting and distinguishing features to merit your interest.   
...
database.  This is especially useful in a web application where the data is often loaded from the database
and then displayed to the user for editing.  If this offends your sense of architectural purity, there are
...
well for many other frameworks such as Ruby on Rails, and I think it's worth considering for Perl as well.

This article will not cover all the ins and outs of L<Valiant> but I will provide a brief overview of the
...
define validation rules for the class.  These rules typically are applied to L<Moo> or L<Moose> attributes
...
This will return a hash:
    ...
    ( email => ['Email is not a email address'] )
...
is covered in the full L<Valiant> documentation. But this gives you they basic idea.
...
If you want full details you can check out the actuall SQL used to create the database in the Agendum
...
'on_hold', 'blocked', 'canceled', or 'completed'.  In addition, once
a task is set to completed it cannot be changed to any other status.
lastly a task that is 'on hold' or 'blocked' can't be set to completed
directly.
...
and addtionally provide extra features to make it work more like how you'd expect it.  We also provide
some additional features to L<DBIx::Class> to make it easier to work with L<Valiant> in the context of
a web application, such as full recursive update and create.  You can check out the core documentation
at you leisure (L<DBIx::Class::Valiant>, L<DBIx::Class::Valiant::Result>,
L<DBIx::Class::Valiant::ResultSet>) , but I'll provide a brief overview here
using code from Agendum.  First here's the Schema class:
...
There's really nothing here directly related to L<Valiant> but I wanted to show you the schema class
...
is useful for tracking changes to the database model.  The C<InflateColumn::DateTime> component is
useful for working with L<DateTime> objects in the database model.
...
Lasting here's the default resultset class, which was mentioned in the Schema class.   This is 
...
As you can see not much going on here.   I know a lot of people set the default resultset class to
the base resultset but I prefer to separate them out.  It's a personal preference.

Ok so far nothing really interesting.  The real magic happens in the result classes.  Lets do the
...
key, and the relationships.  BTW you can use L<DBIx::Class::Candy> with this if you like, my integrations
support that and you can read about it in the docs.  The next part is where we add the L<Valiant> stuff:

    __PACKAGE__->accept_nested_for('task_labels', {allow_destroy=>1});
    __PACKAGE__->accept_nested_for('comments', {allow_destroy=>1});

Since L<DBIx::Class::Valiant::Result> adds support for nested updates and creates (and delete's optionall) we 
...
So in this method we enforce the more complex state transition rules for the status field.  This can
...
This is a bit more complex.  We use the C<date> validator to ensure the value is a valid date, and then
we use the C<min> option to ensure the date is greater than or equal to the current date.  We only
enforce this validation if the due date is changed.  This is useful for web applications where you
might want to allow the user to update a task without changing the due date.
 
BTW sorry about this bit of code:

    min => sub { pop->now->truncate( to => 'day' )->subtract(seconds => 1) },

The 'min' option is 'must be greater than' not 'must be greater than or equal to' so we have to
subtract a second to make it work.  I'm working on a fix for this (probably will just add 'min_or_equal'
and 'max_or_equal' options) but for now you have to do this.  Don't be shocked if you check out the
repo and see a fix in place.

Finally the C<Comment> result class has a validation.  Its not much so I will just show all the code:
...
In Agendum, these validations are run from the Catalyst controllers which gather field info from
a POST request.  However if you are just writing a test script you can run the validations directly
on the object.  Here's an example:
    ...
    if($task->has_errors) {
      my %errors = $task->errors->to_hash(full_messages=>1);
...
This would output something like:
    ...
    Errors: {
      due_date => ["Due date must be greater than $whatever_today_is"]
      comments => [
        "Comments Is Invalid",
      ],
      "comments[1].content" => [
...
calls C<validate> for you.  Same think for C<update>.   If you prefer more control you can disable
...
internationalization, nested creates and updates, using the form builder and more.  If you want to get a feel 
...
also happy to take pull requests if you have a feature you'd like to add.  I hope you find L<Valiant> useful
