...
Have you ever added a print or say statement, only to realize that there
is some reason no output?
(Assuming you have no syntax errors).

Now, instead imagine working in a large, complex codebase where,
...
The problem become to find the place that closes C<STDOUT> to comment it out
for debugging or figure out how to print it still to the screen
