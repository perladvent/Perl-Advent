...
Santa faces some of the same technology issue that many of us has
...
Now he needs to fix up his records some he can
...
That could be it's own Perl program. But, as the data structure gets
more and more complicated, so does the code. Santa is and old-school,
...
The L<https://rx.codesimply.com|Rx language> allows us to easily
...
Once we have our schema, we tells L<Data::Rx> to create the Perl
...
L<https://github.com/briandfoy/cpan-security-advisory|CPAN Security
Advisories>. Each reported CPAN distribution has a file dedicated to
it, and there are certain information we want to collect for each
distribution and for each advisory in that distribution. Some of that
is hand-edited, which inevitably leads to mistakes.

Adding a Data::Rx tests, as in
L<https://github.com/briandfoy/cpan-security-advisory/blob/master/xt/
validate.t|xt/validate.t> and L<https://github.com/briandfoy/cpan-security-advisory/blob/master/xt/
validate-db.t|xt/validate-db.t>, allows us to check every data file for
