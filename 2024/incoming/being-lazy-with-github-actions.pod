...
The article made it seem thatGitHub Actions were just as useful for Perl
...
The app already had a robust set of tests written with Test::More, but running
them manually was tedious.
...
automated the entire deployment process. The `workflow_dispatch` key in the
`on` section of the workflow added a "Run" button to the "Actions" tab in the
