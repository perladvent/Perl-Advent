...
A quarter of a centaury (and, obviously, twenty five days) ago I created the
...
Over this this time the world has changed, and Perl's role in the world with
...
Python, or our beloved Perl - or any number of range of interesting and exciting
...
the ability to easily obtain whatever environment we want to use any
...
want to hand.  This brought the philosophy of "CPAN is the language" to the
forefront.
...
installing a module.  Where we once begrudgingly replied on source code
modifications to manipulate the code we write to add brand new language features
we now exist in a world of pluggable syntax where we have (almost) infinite
adaptability.
...
years we've seen some of these experiments being turned into best practices tha
...
had objects.  If I wanted to represent a triange and try to calcualte its area
...
This is so much better.  We don't have to name the C<area> method with a
C<triangle_> prefix in order to disambiguate it from the any C<rectange_area>
...
(admirably still experiential) of syntax that we started as a community playing
with 2008 with L<MooseX::Declare> - but implemented much better and safer way. 
...
JavaScript's ever popular npm, Pyhton's PyPI, and Ruby's RubyGems as examples.
The concept of shipping these libraries with working tests like everything on
the CPAN has been adopted everwhere, with CI/CD systems validating any serious
...
five years ago are already there waiting for me too.
...
at a Perl Mongers social meeting that there was no good advent calendars for
...
year I introduced a small description of why I'd picked it along with the pid,
...
This turned out to be surprisingly popular, with the module being featured in
...
Avent Calendar at this time of year).
...
any one year, or event any one programming language - is the idea that we can
all enjoy learning about our favorite programming language.  Things don't have
to be taught in boring instructions, but the instead having something to learn
about can be silly, daft, fun, and a treat!  Why not have a story about how
...
person in the Perl community has talked about optomizing projects for fun -
