...
Last year in 2022, Santa Clause discovered a very useful Perl module called
...
method which gives acessors to to C<HASH> references - or the I<inverse>,
...
One is a nifty method called C<opt2h2o>. This method makes it more
...
C<opt2h2o> is used to provide a list of default accessors to C<h2o> to an given
...
He is now able to build up a configuration file considerably more facts for
his I<Did you know ...?> script:

Better yet, you may have not noticed but the script was combine commandline
flag processing already covered, to make it dead simple to use a different
configuration file by specifying the C<--conf> flag.
...
=begin perl
  use strict;
  use warnings;
  use Util::H2O::More qw/ddd Getopt2h2o yaml2h2o/;
...
There are many other cool aspects to C<Util::H2O::More> that Santa wishes
to show you, but he hopes that in time you discover all the time and lines
of code that this module has to offer for a vast number of applications.
...
It build ons L<Util::H2O> massively powerful combination of C<h2o> and C<o2h>
...
=item C<ddd> - a short cut around dumping references to C<STDERR via C<Data::Dumper>
...
=item L<https://metacpan.org/pod/Util::H2O::More>

=item L<https://metacpan.org/pod/Util::H2O>

=item L<https://www.historiesdarkmysteries.com/2021/12/the-surprising-origins-of-jolly-old-st.html>
