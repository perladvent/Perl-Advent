...
recording, playback or other tasks at hand. 
...
Like 'carol', all new tracks start as members of the 'Main' bus. 
...
A chain setup is made of signal-processing I<chains>--edges in
the processing graph--and I<loop devices>, which are nodes
capable of summing their inputs.

Each chain has one input and one output. This is a setup
of one chain, omitting general options related to
buffersize, etc.
...
A chain can represent an I<audio clip>, part of an audio file
that plays at a particular time.
...
starting at 0:80. The 'playat,40' option plays the clip
