...
* Validating file uploads are expected and are the expected type
...
failed - so I approached our friendly CPAN admins for help, obtained co-maint,
and released a new version.  Since then we added more - including recursive 
scrubbing of parameters within POSTed/PUTted request bodies.
...
checking so that if a action hasn't specified that it expects file uploads,
...
read this!) - including the ability to say e.g. C<image/*> for any type of
image, some extra heuristics so that e.g. a shell script and an XML file aren't
both treated as C<text/plain> as the underlying L<File::MMagic> returns, and
the ability to control and/or override the handling of unexpected unloads.
