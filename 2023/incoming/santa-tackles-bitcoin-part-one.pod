...
names Bitcoin as a thing they want to find under the Christmas Tree. This new
trend comes much to displease of the Kriss Kringle.

- This is outrageous! - he yells in annoyance - How can this thing ever have
value? Hasn't it died like five hundred times already? It's not even real!

He starts stroking his beard, thinking. His white moustache dances like waves
on a beach as he tries to understand the new reality of 2023. If the good kids
want it, then he must oblige. But how can something be left in someone's house
...
So he searches the Web for the term, but most of what he gets are the rubbish
price speculation articles. The L<Bitcoin wiki|https://en.bitcoin.it> is of
some help, but it is often outdated. The L<Bitcoin Improvement
Proposal|https://github.com/bitcoin/bips> documents are good, but a bit too
specific for his level. So maybe books? He reads I<Mastering Bitcoin> by Andreas
Antropolus, which gives him a nice insight into the technical side of Bitcoin.
He presses ahead with I<The Bitcoin Standard> by Saifedean Ammous, which is
providing all the economic details he needs.

It quickly became obvious the new task can't be performed in the current modus
operandi. The Elves are used to crafting everything themselves and often
resorting to magic in their toys factory. They can't counterfeit Bitcoin - not
because it's a bad thing to do, but rather because it's impossible. They can
either try to mine it or obtain it by providing high quality goods and services
on the market. Luckily, one of the elves has remembered he played with
bitcoin back in 2011 and still had the old hard drive, from which they
were able to reclaim keys to 50 bitcoins, enough for a big juicy present for
I<a lot> of kids. The unspent transaction ID for their coins is
...
Now the next step is to split the big chunk of 50 bitcoins into smaller pieces.
...
number N is well in hundreds range this year, crafting this transaction by
hand in a wallet software is out of question and you need some kind of
programmable library. McJingles, being Perl lover, decided to give the module
...
So far so good, but there are some decisions to be made before continuing. The
old private key was in I<WIF>, short for Wallet Import Format, which is only
capable to hold a single private key. Now hundreds of private keys need to be
...
but since the P2PK scheme is effectively obsolete it's better to generate a new
type of address and send there. McJingles decided to generate a new mnemonic
...
Mnemonics phrases are sets of words which encode the wallet's entropy in a human
...
the mnemonic to take all the Bitcoin from it. It is a secondary security
feature. McJingles knows it's best to store the mnemonic offline, for example
written on paper.
...
# get the master key for the BIP44 account
# use purpose 84, which will create Segregated Witness addresses
...
do. Adding these outputs turned out to be much simpler than he anticipated:
...
The difference between input and output values is what miner gets paid to
...
mined, but paying over $1.5 million for a transfer is simply too much. The
fee must be proportional to the size of the transaction, so McJingles assumed
100 satoshi per output should be enough. He also sets his transaction up for RBF
(replace-by-fee), so that he can always bump it later if the blockchain gets
clogged:
...
The transaction object is now complete! Now all he had to do is to sign it and
print it to the console. The transaction was not yet sent, but the serialized
hex string can be broadcasted to the Bitcoin network at any time, since the
validity of the transaction won't expire!
...
When run, the program printed the mnemonic phrase to backup, an user-friendly
...
McJingles has already safely backed up the mnemonic, so he decided to write
another script to print all the addresses he used. Running the same script
again would generate a new mnemonic phrase for him and there is always paper
shortage in the toys factory since all the presents must be wrapped due to
company's policy.

It was getting late and he felt exhausted from deciphering all this output. It
would be silly to work with Bitcoin being this tired! Any error he makes may
cause the coins to be lost forever! Well, with great power... yeah, it's best
if he just goes to bed and leaves the rest for another day. Still plenty of
