...
Last day he learned all about how to make a transaction using
...
those Wallet Import Format things he had used before to get access to the
...
spaces while providing the mnemonic for the program. A quick scan though
L<Bitcoin::Crypto::Key::ExtPrivate> documentation has revealed that
C<from_mnemonic> method usage without the language argument is not only
whitespace-sensitive but also doesn't check the if the words exist in the
...
able to move the coins. And so are the children! When they wake up in the
...
spare the kids all the retyping of the WIF it's best to also provide a QR code,
but that's a problem for another day (or another elf).
