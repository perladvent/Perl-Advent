...
That C</u> tells the substitution to use the Unicode rules, which means this also trims all the L<other whitespace that Unicode defines|https://www.effectiveperlprogramming.com/2011/01/know-your-character-classes/>. I'm especially fond of that one after chasing down a bug caused
...
This is such a common operation that it deserves its own name— C<trim>. The L<builtin> pragma is still experimental, so I turn off
those warnings with L<experimental> before I enable C<trim>:
...
There are a few other interesting features for you to discover in L<builtin>, and I hope man of these lose their experimental status soon.
