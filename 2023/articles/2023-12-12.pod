...
And, while I'm very comfortable with references, if I can leave out all the dereferencing arrows, C<< -> >>, I will. When Perl v5.10 added C<state>, it could only initialize scalars. This wasn't a big deal with anonymous arrays, it feels clunky with a hash because I often want to construct the hash to check existence of some allowed value:
