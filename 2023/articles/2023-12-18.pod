...
Title: Trimming audio files with Nama
Topic: Audio::Nama
...
heroic pranks of Christmases passed, Gringle Graybeard, one of
the IT elves, has been piping music to the office, canteen
and tool benches using cabling left over from an obsolete
phone service.
 
The elves have gotten into radio plays,
and Gringle wants to edit some old recordings to remove
unwanted segments. In other words, he wants to extract
the relevant parts of an audio file, stitching them together
into a new file. Gringle runs Linux and prefers to do his work
in the terminal whenever possible. We've decided to help.  

=head2 Our tools

We'll be using L<Nama|https://metacpan.org/dist/Audio-Nama/view/script/nama>,
a multitrack recording, mixing and audio-processing
application written in perl, and
L<Ecasound|https://ecasound.seul.org/ecasound/>, a
...
spreadsheet recalculates as necessary. Once the engine is
configured and ready, it can receive realtime commands
to start and stop the engine, seek, or modify the
parameters of signal-processing plugins such
as a volume control. 
...
At this stage, one end of each edge is a track. An
...
This is used in mixdown and other situations where the
values needed differ from those of the parent track.
...
chain identifier that is not a track number. It stands out.
