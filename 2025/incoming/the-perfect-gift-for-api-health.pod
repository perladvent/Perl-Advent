...
=begin perl
  ...
  if (@schema_errors) {
      print "🎅 Uh oh! Found missing reindeer (schema errors):\n";
      foreach my $error (@schema_errors) {
          print "   🦌 " . $linter->format_schema_error($error->{message}) . "\n";
      }
  } else {
      print "✅ All reindeer present and accounted for!\n";
  }
  ...
  if (@linting_issues) {
      print "🔧 Some toys need polishing:\n";
      foreach my $issue (@linting_issues) {
          my $emoji = $issue->{level} eq 'ERROR' ? '🎄' : '🌟';
          print "   $emoji [$issue->{level}] $issue->{message}\n";
      }
  } else {
      print "🎁 All toys are perfectly crafted! Ready for delivery!\n";
  }
...
Output when checking our naughty API:
  ...
  Checking if all the reindeer are present (schema validation)...
  🎅 Uh oh! Found missing reindeer (schema errors):
     🦌   - /info/version: Missing property.
     🦌   - /paths: Missing property.

  Checking toy quality (linting issues)...
  🔧 Some toys need polishing:
     🎄 [ERROR] Missing paths
...
Output when checking our nice API:
  ...
  Checking if all the reindeer are present (schema validation)...
  ✅ All reindeer present and accounted for!

  Checking toy quality (linting issues)...
  🎁 All toys are perfectly crafted! Ready for delivery!
...
=begin perl
  ...
  print "🎅 Santa is checking his list...\n";
  ...
  if (@$errors) {
      print "❌ Found naughty APIs!\n" . join("\n", @$errors);
      print "\nBetter get them fixed before Christmas!\n";
  } else {
      print "✅ All APIs are on the nice list!\n";
...
=begin perl
  ...
  print "🎄 Sorting through the ornament box...\n\n";
  ...
  print "🔴 Red bulbs (errors): " . scalar(@red_ornaments) . "\n";
  print "✨ Silver tinsel (description warnings): " . scalar(@silver_tinsel) . "\n";
  print "❄️  Snowflakes (path issues): " . scalar(@snowflakes) . "\n";
...
=begin perl
  ...
  print "🎄 Preparing the Christmas API feast...\n";
  ...
  eval {
      ...
      if (@schema_errors) {
          print "   ❗ The turkey is undercooked! Schema issues found:\n";
          foreach my $error (@schema_errors) {
              print "      🦃 " . $linter->format_schema_error($error->{message}) . "\n";
          }
          print "\n   🚨 Can't serve the feast with schema errors!\n";
          exit 1;
      }

      print "   ✅ Turkey is perfectly cooked!\n\n";
      ...
      foreach my $issue (@issues) {
          if ($issue->{level} eq 'ERROR') {
              $lumps_in_gravy++;
              print "   ❗ LUMP IN GRAVY: $issue->{message}\n";
          } else {
              $missing_spices++;
              print "   ⚠️  NEEDS SPICE: $issue->{message}\n";
          }
      }

      if ($lumps_in_gravy) {
          print "\n   🚨 $lumps_in_gravy lump(s) in the gravy - feast is ruined!\n";
          exit 1;
      } elsif ($missing_spices) {
          print "\n   ⚠️  $missing_spices side(s) need more spice, but feast is edible!\n";
          print "   Consider adding more flavor for next year.\n";
          exit 0;
      } else {
          print "\n   🎉 Feast is perfect! All dishes are seasoned just right!\n";
          print "   Merry Christmas and bon appétit!\n";
          exit 0;
      }
  };

  if ($@) {
      print "\n🔥 The kitchen is on fire! Failed to process $ARGV[0]: $@\n";
  }
...
C<OpenAPI::Linter> automatically detects and supports all the hard-working elves in your workshop:
  ...
  🎅 OpenAPI 3.0.x Workshop: 3.0.0, 3.0.1, 3.0.2, 3.0.3
  🦌 OpenAPI 3.1.x Workshop: 3.1.0, 3.1.1
...
=head2 🎅 Merry Coding and Happy Holidays! 🎄
