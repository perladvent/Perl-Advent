...
Chaz also had to consider the fact that the master file of kids can be sent in batches of a few hundred records at a time so he had to the ability to pass some argument options to allow for defining a filename as an argument and to select whether the output file will contain Naughty or Nice kids. 
