Author: Sawyer X <xsawyerx@cpan.com>
...
=head2 🎁 "Why are these C<SELECT>s so slow?" cried an elf.
...
=head2 ✨ Deployment at the North Pole
...
=item * B<< 400 milliseconds → around 10 microseconds >>
...
=head1 🎁 And Now: How You Can Use Melian
...
=head2 🎄 Step 1: Create a Tiny SQLite Database
...
Instead of a config file, we will use environment variables:
    ...
    export MELIAN_TABLE_TABLES="gifts#1|60|id:int;recipient:string"
...
=head2 🎄 Step 4: Query Melian from Perl
...
I<(Yes, the elves use Perl too - it's very popular north of 89° latitude.)>
    ...
    # Fetch by numeric key: table ID, column ID, key
    my $gift = $client->fetch_by_int( 1, 0, 2 );
    say "Gift for $gift->{recipient}: $gift->{gift} (€$gift->{price})";

    # Fetch by string key: table ID, column ID, key
    my $bob = $client->fetch_by_string( 1, 1, 'Bob' );
...
=head2 🎄 Bonus: Ephemeral Derived Tables
...
Melian doesn't require the table to exist physically — it just needs a C<SELECT> that returns rows.

This lets you use Melian as a B<lightweight materialization layer> without touching your source schema.

=head1 ☕ Wrapping Up (with Cocoa)
