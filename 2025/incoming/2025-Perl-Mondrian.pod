...
then wrote my <a href="https://www.amazon.com/dp/1736421409">first novel</a>, and later painted 
...
reality as possible</b>? No? Well, maybe you've seen his work:
...
<a href="https://inventwithpython.com/bigbookpython/project47.html">in Python</a>, and I liked their
basic approach--use some configurable random-numbers generators to generate a virtual canvas.  But
let's extend that some in the Perl instance. I'd like to have it working with Mondrian-correct
colors, and be able to output JPG, PNG, and SVG instead of the terminal. I'll use
L<Moo|Moo>, mostly because I like Moo, and it makes this
  ...
  package Acme::Mondrian::Generator {
     ...
     has palette => (
        ...
        default => sub {
           ...
           # RGB numbers.
           return {
              white  => [ 245, 242, 235 ],        # lead white approximation
              black  => [ 30,  30,  30 ],         # bone + carbon black
              red    => [ 138, 47,  47 ],         # hematite / iron-oxide red
              yellow => [ 255, 210, 30 ],         # mix: cadmium yellow / ochre / chrome yellow
              blue   => [ 200, 210, 210 ],        # pale blue-gray (white + carbon black)
           }
        }
     );
...
I suppose, but I'll start with horizontal lines, then do verticals:
  ...
  my $numberOfSegmentsToDelete = 0;  # I'll explain in a minute, really.
...
Now, L<Perl::Critic|Perl::Critic> fusses at me here about redefining C<$x> in the same lexical scope. Which
I did, even though I was done with the B<first> C<$x>. To keep the Critic and 
L<Test::NoWarnings|Test::NoWarnings> happy,  let's redo this with some lexical blocks:
...
of line at a time.
  ...
  DELETE: for (1 .. $numberOfSegmentsToDelete) {
      while (1) {  # let's look for a black spot on the canvas.
          my $startx = int(rand($width - 2)) + 1;
          my $starty = int(rand($height - 2)) + 1;
...
painting. Now we need an output method. Nothing fancy here, but L<Imager|Imager> will let you output
either jpg or png, by just specifying the filename you want to use.
  ...
  sub save_png_or_jpg {
     ...
     for my $k (keys %canvas) {
        ...
        $img->box(
            xmin=>$x*$scale, ymin=>$y*$scale,
            xmax=>($x+1)*$scale-1, ymax=>($y+1)*$scale-1,
...
default behaviors, and have it output a PNG. 

<img src="mondrian_in_perl.png">
...
L<Acme::Mondrian::Generator|Acme::Mondrian::Generator> in the next week or so), 
I'll also include a script to let you play around with the parameters of the
generator, and output JPGs and SVGs as well as PNGs.
