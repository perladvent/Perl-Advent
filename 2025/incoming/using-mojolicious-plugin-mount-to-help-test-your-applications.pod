...
present a simple trick that take advantage of the L<Mojolicious::Plugin::Mount>.
...
But when the module or application make multiple requests to different
external services that could be challenging.

The idea here is to mock the external services instead.

=head1 How C<Mojolicious::Plugin::Mount> can help
...
You can check their routes by doing:
...
By using C<Mojolicious::Plugin::Mount> you can glue them together doing:
...
Here an example, considering C<foo.pl> and C<bar.pl> mocks above:

Consider the main application below:
...
enviroment so the main app can get them.
...
You can mount different mocks dependeing on the need of each test.
