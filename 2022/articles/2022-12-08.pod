...
=head2 A Do-it-Yourself Tree with perl
...
Perhaps a little explanation to the policies set with C<< pEFL::Elm::policy_set() >>: this causes the Mainloop started by C<< pEFL::Elm::run() >> to quit automatically when the last window of the application is closed.
...
Again we register a callback to the following event handler that creates the SVG file and let the viewer show
