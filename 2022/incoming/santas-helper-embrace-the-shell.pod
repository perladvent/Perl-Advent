...
=head1 Santa's Helpers Embrace the Shell

Santa's workshop is very busy and everyone on staff is expected to contribute beyond just their traditional roles. Even Santa's reindeer take on additional tasking throughout the year.
...
To improve their real-time efficiency when it comes to irregular activity, debugging, and general proof-of-concept research, everyone has been working to improve their command line fu. Much of what they do involves working with structured data, such as JSON files.
...
Now they could easily pipe that to grep ro remove the naughty folks, but we can do better better than that, and avoid skipping any nice children whose name contains the word naughty.
...
C<jp> is a very new tool and the elves are the first in history to discover its existence. They understand there may be bugs or additional functionality that it lacks, and they are excited to make contributions to improve C<jp> and share those improvements with all the children and everyone else in the world. They have confidence in the tool and their ability to make contributions because it is pretty well documented and comes with a very easy to use test suite that already tests most of C<jp>'s functionality.
