...
Because there is no time for long coding during the jolly season, let's just
grab some example from L<Chart::Manual::Types>. Every line of it is
explained there. Once the code is pasted into the editor, we change the first dataset 
(animal names => gift recipients) and paste into the following sets our numbers.
At last we label axis and legend properly and a first draft is ready:
...
The values under the key C<colors> just gray out the less important parts,
...
To change that we load the whole toolkit (a direct dependency of C<Chart>) and import
also the helper function C<color>, to keep it all short and sweet. 
...
create a harmonious chart. For that task we create complementary
colors, which are maximally different in hue, but same in lightness and saturation
(unless the advanced features of the method C<complementary> are used).
...
Here I choose a color between C<'royalblue3'> and C<'royalblue4'> and the
two complementary colors of that.
