...
All the elfs know: it is mainly about spreading joy. But some numbers need
to be kept in check - maybe with bar charts.

=head2 Get some code

Because there is no time for a long coding session during the jolly season,
...
If you found that interesting: there  are still more methods and options
