...
Artie noted to Santa that Pure Perl scripts can be wrapped with an existing version of APPerl, off the shelf builds of APPerl can be found at L<https://computoid.com/APPerl/#RELEASES> .
...
Artie edited C<default_config> in the newly created C<apperl-project.json> to copy in the C<cameltype> script and set the destination executable name to C<cameltype.com>, the name is important as C<argv[0]> is used to determine what script embedded in APPerl to run.
