...
sound everywhere, the Christmas goose has already been ordered, but wait... A very important detail is missing. Rudolph, the Christmas grouch, did not buyed a Christmas tree!! And so his wife Fjolla, a Christmas elf, asks him to finally get a tree. The same procedure as every year...
...
If you want to deep into programming with pEFL see the explanations at L<pEFL>. As the API of pEFL is - apart from his object-oriented interface - deliberately kept close to the Elementary C-API, the documentations at the L<Enlightenment docs|https://www.enlightenment.org/develop/legacy/api/c/start#> should help you, too. The Perl method names usually remove the prefix at the beginning of the C functions. Therefore applying the C documentation should be no problem.
