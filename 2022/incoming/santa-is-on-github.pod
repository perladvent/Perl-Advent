...
Every once a while, Santa -- or the elves themselves -- want to check produced items, so they use Pithub for that:
...
And it produces for instance this kind of output:

	A bicycle for Tom
	A barbie for Lisa
	A tranformer for Billy
...
It seemed at first to work well but, sadly, even though Santa and the elves could still access the repo in GitHub UI, they were no longer able to list the commits via Pithub.

They first started to add some error handling:
...
And this confirmed that the repository is "not visible" with C<< Error: 404 Not Found >>.
...
To materialize when a toy is about to be delivered, the elves are opening pull requests, and they once again use Pithub for this.
...
It's a little bit of work, but once again very easy and straighforward:
...
Thank to Pithub, elves were able to easily interact with GitHub API for both reading and writing.

And if Christmas is once again a success this year, it would deserve some credits to L<Pithub|https://metacpan.org/pod/Pithub>! 
