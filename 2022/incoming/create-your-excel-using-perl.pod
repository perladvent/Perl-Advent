Author: Csaba Simándi <1x1m4nd1@gmail.com>
...
There were only 3 days until Advent, with lots of articles missing, so the Wise Old Elf immediately delegated this task to Wunorse Openslae, and he choose wisely - as always. Wunorse was a master of managing Excel files and started to work on that article immediately.

=head2 Preface

Some historical background: The older module L<Spreadsheet::WriteExcel|https://metacpan.org/pod/Spreadsheet::WriteExcel> is for creating Excel files compatible with Excel 97, 2000, 2002, 2003 and 2007, mainly with C<.xls> extension. It is mostly compatible with Excel::Writer::XLSX, so L<migrating to the newer version|https://metacpan.org/pod/Spreadsheet::WriteExcel#Migrating-to-Excel::Writer::XLSX> should be as easy as a christmas pie. However such a migration is more like a heavy christmas pudding loaded with goodness and lots of issues might arise during the process.

The old module comes along with C<Spreasheet::ParseExcel> just to name an example for reading Excel files. You can use C<Spreadsheet::ParseXLSX> or C<Spreadsheet::XLSX> to open and manipulate existing Excel files. This article deals with creating new Excel files.

=head2 Basics

First things first: we need to import the module, initialze an xlsx file, add some worksheets, input some data on them and finally close the file, as we would do with a usual filehandle.
...
We can put everything else in between, depending on Santa's needs, a list of Christmas cookies from around Europe in hash format, is a good example, including some stats.
...
The rest depends on your choice from the two options. It is more convienient - in our opinion - to create a format object with a hash, rather than calling the formatting methods afterwards.
...
You will need to pass the format object to Your write method as parameter:
...
First of all we need some data, let's compose our cookies list.
...
If You pass and array ref of array refs to C<write()>, it is the same if You would call C<write_col()>, it will write to the columns, as demonstrated in the lines 22 and 27.
...
To cover all of it's features would take a whole book.
