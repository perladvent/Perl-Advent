...
I<"Sure we will."> Binky replied without taking her eyes off the monitor. I<"We'll control what prints what on the .dataprinter file">. She created a new file called ".dataprinter" on the root of the project path, then made it look something like this:
...
Finally, calls to C<p()> made inside C<Reindeer.pm> would not attempt to stringify objects that have a subroutine `as_string`, `to_string` or similar, nor will they show wrapped method modifiers or methods inherited from parent classes. Even so, that particular output is in a hot codepath and can get pretty big, so instead of printing to STDOUT, it will write to the custom C</var/log/christmas/reindeer.data> file.
...
I<"You're gonna play with the output colors and make a new theme for Data::Printer now, aren't you?">
