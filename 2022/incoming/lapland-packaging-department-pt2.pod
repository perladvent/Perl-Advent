...
=head1 Lapland Packaging Department - Part 2

After getting to work, Tonttu quickly downlaoded the Carton module's tarball.
This was the chosen one to make the module installation and packaging easier.
Carton is capable of dependency handling, can download the source tarballs for modules and last but not least, can create a portable package for any project This package can be copied to any machine and Carton will install all modules automatically, without the need foran internet connection.
He started to do some testing and cooked up the follow proof of concenpt for Santa for the first part:

Tonttu made sure first, that Carton is installed on all servers in the warehouse. He used the manual compilation method, he jotted down the day before. 

After that he  wrote the following guide for the busy elves:
