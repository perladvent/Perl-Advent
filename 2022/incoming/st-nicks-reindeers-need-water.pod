...
Topic: Perl OOP, HASH references, accessors

=encoding utf8

=head1 St. Nick's Reindeers Need H2O!

Reindeer are generlly hardy creatures, but the ones Santa chooses
...
Santa thinks this is such a nice interface for Perl C<HASH> references, he might
even be able to convince the Easter bunny to finally get off His cotton tail and
finally create that centralized database they and the Tooth Fairy can use all year
long ... yanno, for good stuff I<only>.

=head1 Conclusion
  
With St. Nick's list now complete with online usage profiles for all people,
he now sets his mind on more jolly thoughts; not sugar plums and candy canes,
but all the applications for which he may use C<Util::H2O> to make working with
