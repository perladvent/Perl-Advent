...
=head1 MooseX::Getopt saves Christmas
...
He sighed. "Fixing the new delivery planning script. I<Someone> committed it without even basic usage docs." Ernest opened a new <delivery_plan> file for the script itself. "Here. Watch.".
...
Ernest nodded, "In one. And it also gives you C<--nonice_check> for free because it's a C<Bool>."
...
"If its a more complex type and the behaviour on the parent type isn't enough, there's a helper function to update the type map, C<add_option_type_to_map>, but you can read up on that in the module docs yourself." Ernest grinned at her. "Now scoot, while I test this and push it." 
...
Ernest fired off an email.
    ...
    When you wake up, please check the commit log for C<delivery_plan>, and
    learn from it. The Big Red Boss doesn't have time to read source code
    to figure out what things we give him are meant to do.

    Also, how on Earth did you managed to come up with an O(N) solution
