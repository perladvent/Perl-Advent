...
Topic: AnyEvent
Author: Augustina Ragwitz <augustinalareina@gmail.com>
...
going on:
  ...
  while (my ($service_name, $url) = (each %{$self->urls})) {
    ...
    # before the condition variable is "done".  i.e. let the condition variable
    ...
    # and we've got th content, call the callback.
    my $request;
    $request = http_request(
      GET => $url,
      timeout => 2, # seconds
...
bit by bit once we've learnt a little more about AnyEvent.
...
just one <Ccondvar> and then specify when you want to set its ready state so
