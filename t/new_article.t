...
use Test::More import =>
    [qw( BAIL_OUT done_testing is like ok require_ok subtest )];
...
subtest 'slugify_title' => sub {
    my @cases = (
        ...
        [   '  Hello, Perl Advent!  ',
            'hello-perl-advent',
            'trims punctuation and edges'
        ],
    );

    foreach my $case (@cases) {
        my ( $input, $expected, $label ) = @{$case};
        is( Perl::Advent::new_article::slugify_title($input),
            $expected, $label, );
    }
};
...
subtest 'script execution' => sub {
    ...
    my @cmd = (
        $^X,       $script, '--title',  $title,
        '--topic', $topic,  '--author', $author,
    );
    is( system(@cmd), 0,
        'new_article creates file in current year by default' );

    my $path
        = catfile( $tmp, $current_year, 'incoming', 'openapi-linter.pod' );
    ...
    @cmd = (
        $^X,        $script,         '--year',  $override_year,
        '--title',  $override_title, '--topic', 'Some::Topic',
        '--author', $author,
    );
    is( system(@cmd), 0, 'new_article supports explicit year override' );
    ok( -f catfile( $tmp, $override_year, 'incoming', 'year-override.pod' ),
        'article created in override year' );
