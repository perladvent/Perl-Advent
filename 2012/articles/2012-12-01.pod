...
File and directory paths. They begin so simple, but then depending on which
system the script run, they get to use slashes or backslashes. And then there
are spaces that comes to mess everything up. And then there are those systems
that use drive letters and what's not. And so on, and so forth.
...
not want to do that. Travelling up and down directory structures will
now be a joy:
...
entries or traverse the directory structure? All there:
  ...
  say { $wishlist_filename } "I want a $_" for @gift_ideas;
...
probably wish they were also availabe as L<Moose> types to use them everywhere
natively.

And then you'll discover L<MooseX::Types::Class::Path>, and the
