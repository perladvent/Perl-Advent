=head1 Fixing the regexep code block facility
...
The second big issue was that by just saving the pad and resurrecting it
from time to time, lexicals at best did the wrong thing, and at worst
caused segfaults. In particular, the behaviour of closures didn't match
reasonable expectations. For example, this code:
...
all Just Do the Right Thing.

However, Perl also supports patterns that are determined at run-time, or
which contain a mixture of compile- and run-time patterns, such as
