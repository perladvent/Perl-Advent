...
Instead, I wanted something where I could list the possible values for each of the positions, but without having to make all of the combinations myself. That is, I wanted a way to take a cross product of sets where each result would have one element from each set. Hence, L<Set::CrossProduct>.
...
The L<Set::CrossProduct> module isn't the only way to accomplish this task. The L<Algorithm::Loops> module has a C<NestedLoops> subroutine that can do the same thing.

Some people confuse a cross product of different sets with permutations of elements in the same set. If that's what you want to do, L<Algorithm::Combinatorics> might be the right tool.
