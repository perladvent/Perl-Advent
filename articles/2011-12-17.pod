...
fails, set and error and return undef.  Otherwise, return the result we got.
...
Now we know that C<eval> will always return 1 unless it fails.  This means we
need to move the assignment to C<$result> inside the eval, and we need to move
the declaration to an new, earlier statement.
