...
However, in a busy application C<ping()> can get called I<a lot>.
...
to 100K pings I<per hour>. For something that just returns true 99.9*% of the
time, it seems a bit silly.
