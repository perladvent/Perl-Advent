...
Have you ever been installing an a Perl module and wondered why the heck a
...
modules need to be installed starts with my C<cpanfile> file.
...
and browse the code of everything on the CPAN, at its heart an Elastic Search
...
sits on top of the elastic search database.
...
The code to look up the distribution a module comes in is straight forward:
...
Using these two methods can then easily build a hash mapping dependencies:
...
The actual code to build the graph is fairly straight forward:
