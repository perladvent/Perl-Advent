...
I intruduce a technique. That is "using epoch as score".
...
Generally, key management of the KVS is very difficulst and confusing (easy to conflict, typo and so on). KVS is very cool, but it is "super global variables store"  in almost, so we must appropriately manage them.
