...
Christmas party, and since this is a modern outfit (as you know we have 
the the largest toy and child database in the world), cutting some paper 
snowflakes just wasn't going to impress.

=for html
  <center><img src="screenshot800.png"/></center>

I decided to write a little Perl program to render these three dimensional
snowflakes and make them spin around on screen.

=head2 Roles

The first step was to create a class that represented a snowflake, and knew how
to draw it.  As the rendering of the snowflake is distinct from the actual
attributes of the snowflake itself, I decided to create separate roles for the
model and drawing code and combine these together in the end into one concrete
snowflake class.

So, starting with a model to represent the physical characteristics of each
...
Over our long summer break this year, an aquatic mammal who was 
...
The next piece of code creates an instance of L<FFI::Platypus> which we 
will use to create the bindings to the library:
...
is smarter about finding I<dynamic> libraries.  We have a mixed 
environment of OS X and Linux for toy deliveries and testing, so it
...
hardware types like "sixteen bit signed integer", Platypus encourages 
...
an overhead to creating objects for single use calls like this, and it 
...
Many libraries implement hundreds of functions and keeping C<@EXPORT> in 
sync with functions as we add them is very tedious.  Instead we use 
...
will need.  A few more specialized types, like arrays of C strings
...
you have to provide the size of the array as an additional parameter.  In
...
ignore any changes made by Glut, the scalar C<$size> B<is> updated if 
...
Or calling back into Perl from C.  This is extremely useful, but it is 
...
language itself takes responsibility for deallocating objects when they 
...
keep track of whether or not the closure is still in use by Glut!  In 
