...
they are going around with this outsourcing in a indiscriminate manner; the Big
Man in Red made it very clear that toys should not be bought from big
mega-corporations, but only from Mom'n'Pop shops, where they are build out of
...
packaging with air holes (so the bears can breath) and each bear gets free
...
able to disgard any result which the passed subroutine doesn't return true for:
