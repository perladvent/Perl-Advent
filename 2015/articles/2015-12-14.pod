...
There was a time when, like a chump, I'd write statments like this thoughtout
my code:
...
This I<poor man's debugger> technique is supprising effective.  Unlike a real
...
Lazyness is one of the virtues of a Perl programmer.  Surely we can automate
this?  You bet we can.
...
Sublime Text 3 has a complex plugin system written in Python in which I've
written many a tool.  But, fear not, I need not concern myself with a non-Perl
language in this Advent Calendar entry, for many existing plugins that can shell
out exist that can be installed with Package Control without having to write a
line of code.  Using the
L<External Command|https://packagecontrol.io/packages/External%20Command> plugin
...
=head2 Configuring Your Terminal Emulatator

So, why isn't running this in an editor sufficent?  If you only ever edit code
...
the terminal is outputting and and anything they output is typed back into the
terminal.
...
withing the C<Keys> tab of the application's preferences:

=for html
  <center><img src="iTerm2.png" width="500" height="294"></center>

=head2 Making A OS X Service

What if we want to take it even further?  Have a keyboard shortcut that doesn't
insert the text in a particular appplication but inserts the text I<anywhere> on
...
corner of the screen and selecting them from the C<Services> menu underneith.
...
a new service.  Seach for the C<Run Shell Script> action then configure it to
...
=head2 Keybord Maestro

You'll notice we If you were paying close attention you'll notice that
...
utility that can be used to control your Mac, and one of the many things it can
...
on your keyboard being pressed, meaning that the output from that script can
be typed anywhere in any application running on your Mac, if its explicitly
expecting it or not.
...
program I can configure my computer so every time I type C<metaqq> it deletes
...
intalled Acme::MetaSyntactic themes:

=for html
  <center><img src="km.gif"></center>

In order to work out what was typed in the script we need to set a variable
to the C<%TypedText%> temporary variable, and then access it via an enviroment
