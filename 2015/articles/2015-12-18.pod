...
Likely, Sereal can dedupe hash keys. And it does it almost for free. Moreover, it can dedupe values with moderate penalty. As result, particular key of value string will be dumped only once; all other occurences of same string is encoded as 2-3 bytes sequence.
