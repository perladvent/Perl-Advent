...
with it to pretend to be him in Perl:
    ...
    # connect to the websocket and handle any messages via callback events
    sub rtm_start($url) {
        my $id = 1;

        # connect to the websocket URL asynchronously, and when we connect (or
        # fail to conncet for any reason) run the callback registered to start
...
Jen didn't seem to appreciate his clever solution when he bumped into next to
...
the secret slackbot URL for his account on the slace web pages and make
asynchronously posts to that whenever he wanted to notify himself.
