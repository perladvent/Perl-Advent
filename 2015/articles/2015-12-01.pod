...
children - far from it, their happyness was why he did what he did after all -
but now there were more than ever of them.  World population had grown to the
point where there were 2.2 billion children that potentially wanted a gift, and
...
unprecidented step of installing agents in homes around the world - elves that
sat on shelves - who were able to report on any problem, from waking children to
adverse weather conditions, that might slow him down.  Traditionally an Shelf
...
Santa, so many houses had internet connections the Elf could use to report
...
estimatation all I have to do is take the location from each report and feed it
...
instinctivley knew the way to every chimney top, but useless for plotting on a
map.
...
MaxMind offer the GeoLite2 databases which anyone can periodically download
...
Now the hard work is out of the way, Candy Cane had the relatively simple job of
...
that used the MapBox API would be sufficient:
    ...
    my $MAPBOX_TOKEN = 'pk.eyJ1IjoiMnNob3J0cGxhbmtzIiwiYSI6ImNpZmw1ZzdnMTV5aXBpdWx4dDhpbjF6ZGQifQ.C3lPhZeBCapfVSQhpcGsLA';
