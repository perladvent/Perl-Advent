...
language features to all and sunder.
...
new standards to scripts while at the same time not requiring old code have be
changed to work with the changes if we don't want to.  With experimental
