...
every yearly release, offering its beloved for years to come with new features
and capabilities in the core language itself.

Recent releases of perl pack a lot of features than aren't available by default
(to preserve backwards compatibility), but can be easily enabled by using the
C<feature> keyword.  They even ship with many experimental features that give us
a glance of Perl-yet-to-come!
...
This uses five of the thirteen features available in Perl 5.22
...
* The C<postref> feature (still I<experimental> in 5.22)
...
My favorite feature so far, subroutine signatures, are a still experimental
...
C<lc> the strings.
  ...
  #!perl
  # for these characters only lc works
  lc("\x{212A}") eq lc("K");  # true!
  uc("\x{212A}") eq uc("K");  # false!

  # for these characters only uc works
