...
environment variables before we run the script.  All of these things are awesome.
...
However, both of these are much harder to do than writing a program that parses
simple command line options - there's event loops to consider, callbacks,
possibly listening on various ports.  Maybe there's a better, er, I<choice>.

=head2 Introducing Term::Choose
...
a remote server you're connected to over ssh without the overhead and the
