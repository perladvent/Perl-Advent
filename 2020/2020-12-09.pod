...
Topic: Text::Unidecode
Author: Mark Fowler <mark@twoshortplanks.com>

For all its merits, sometimes we just don't want to deal with Unicode.
We're often in a situation where by we want to have a plain old ASCII
version of what we're looking at.  LE<eacute>on gets annoyed when he
has to use "Leon" instead, but everyone knows what it means.

What B<Text::Unidecode> does is convert a random string into just
...
disagrees on what maps to what.  The fact is that it I<works>
however, and that's what counts.

There's not really much that can be explained about this module
that a simple example won't cover:
