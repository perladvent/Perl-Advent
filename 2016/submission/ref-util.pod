...
new keywords.  So, as our above example:
...
Can much more succinctly be written with the new C<is_blessed_arrayref> keyword:
...
The Ref::Util module introduces new B<keywords> not just new I<functions> using
Perl's pluggable keyword infrastructure.  The guts of the checks are implemented
in fast C code (and skipping all the unnecessary string construction) - and
since the function calls in our code get compiled using custom op codes into a
single op in the tree means it's just as fast as the normal Perl inbuilt
functions without the subroutine calling overhead!

While Santa is not exactly sure what all this means, an elf helpfully explained
that "it makes the sled go faster." And we don't even need to put racing stripes
on the sled!

Faster and less error prone.  Just what the man in red ordered.
