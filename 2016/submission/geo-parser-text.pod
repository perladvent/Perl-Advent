...
There are also a few other current projects: (geolocate from Tulane University, CLAVIN - Cartographic Location And Vicinity INdexer, and mordecai
 https://github.com/openeventdata/mordecai ) All of them focus on extracting place names from a piece of text, geotag, disambiguate, resolve them to the correct place, and return their coordinates and structured geographic information.

I don't know of any geoparser that goes beyond place names, by that I mean one that geoparses street addresses and street intersections too. 
Just like Santa, maybe it does exist. Maybe not.  Either way, I will try to build it.

L<Geo::Parser::Text> interfaces with http://geocode.xyz, a geoparser written in Perl, to disambiguate and extract complete location information from text (locations expressed as combinations of street names and place names.) 
...
Knowing for certain, now that is a problem nobody quite knows how to solve. That includes me. For that I have a probabilistic model that computes a 'confidence score' which takes values between 0.1 and 1, with 1 being the highest confidence score. (A confidence score of 0 may also be computed, but that result is never returned.)
