...
from C<Magpie::Transformer::TT2> which handles building the L<Template
Toolkit|https://metacpan.org/release/Template-Toolkit> object for us. We just
need to provide some callback hooks. First C<get_tt_conf> provides the
...
(ususally a C<Plack::Request> object), and the Resource object (something
...
classes inherit directly from teh C<Magpie::Transformer> class. This means
...
this pipline stage and the order in which to call them.

Assuming we have our C<.po> files and what not setup properly, this will take
any string in curly brances and replace it with the appropriate translation.
...
tranlation strings so that things like the coal count can be translated
properly. But this illustrates how a pipeline of pieces means that as we step
through each piece of the application we can focus donw and work on each step
individually.
