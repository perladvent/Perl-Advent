...
considered the effect this would have on the poor programmer-elf would have to
implement the code to do this!  Case in point the simple code Happy had written
...
Simple functional code.  Get all presents asked for by all the nice children.
...
Feeling there must be a better solution Happy scoured the CPAN till he found the
...
first stab was to pull out the arguments into an array that he built up in
advance:
...
line; Only at that point can they go back and make sense of why @args is set up
the way it is.
...
Happy wasn't, well, happy, about this code.  All those brackets weren't only
...
`$gift->large` in brackets or not?

Now he had List::Gather in his toolset Happy was able to do something much
more readable:
