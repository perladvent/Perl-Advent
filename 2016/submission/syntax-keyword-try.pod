...
As we previously mentioned there are more than.  Let's see how
Syntax::Keyword::Try holds up against the possible solutions.

=head3 Comapring Syntax
...
Some simple L<benchmarking|benchmark.pl.txt> shows that Try::Tiny is very slow,
eval is fast, but the other two modules come in the same order of magnitude as
eval.
