...
=item Look at what's going on. Check out its inputs and use them to dig a
bit deeper using plumbing commands, set the stage.

=item Do the real work. Change log messages, rearrange files or create
new ones, do lots of different things.

=item Return, possibly with a message, including an failure or success
