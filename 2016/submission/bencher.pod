...
How do we run this scenario module? Install the L<bencher-bpm> script from the
L<Bencher-BPM|https://metacpan.org/release/Bencher-BPM> distribution:

   % cpanm -n Bencher::BPM

then run:

   % PERL5OPT=-Ilib bencher-bpm -c 100000 Levenshtein

The result will be identical to the first script we saw, because C<bencher-bpm>
also uses L<Benchmark> to benchmark the codes:
...
distribution) instead of C<bencher-bpm>. Install it first from CPAN (might take
...
the benchmark runs (at least ~1 minute because Text::Levenshtein can only
perform ~50k calculations per second). By contrast, if you look at the
C<samples> result field, you'll see that Dumbbench only needs about 20 runs for
each participant. You actually do not need to set count parameter because it
will figure out the minimum sufficient number of runs.
...
benefit of doing this is that can we can add more data to test easily. Let's say
we want to test some longer word. We'll just add this to our C<datasets>:
...
There's now a C<dataset> result field since there are more than one dataset that
we run.
