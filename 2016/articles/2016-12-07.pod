...
usually run from th command line. However, these are precisely the kinds of
...
Let us see how it works in practice.  Here is an example a simple three line
...
No Perl yet, but just wait there and you will see! To install this three line
script as a hook the three lines should be saved as a file called C<prepare-
commit-msg>, that file needs to be C<chmod +x>'ed to become executable, and
finally that file must be placed into the C<.git/hooks> directory.
...
(this will work as long as the filenames do not have any whitespace in them).
Anyway for every file modified it will proceed to run C<perl -cw> on it. If it
passes muster, it will give the go ahead, it does not, it will exit with a 1,
which will indicate git the commit cannot proceed, printing at the same time the
cause of the error.
...
they unify the two files we have above in a single one.
