...
to simply sufficiently isolate a process other processes running on the same
operating system beyond the normal process management Linux provides.

Currently in Linux there are six namespace which we can control to allow
processes to:
...
UTS - have different hostname and domainname
...
supplying the path to the C<ipc> file descriptor (C</proc/$PID/ns/ipc>). If we
were for example to enter the network namespace, we would use C<CLONE_NEWNET>
