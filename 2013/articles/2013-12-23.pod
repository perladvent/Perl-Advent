...
seconds apart, but there's no way to get the C<$difference> object to
tell you this.
...
C<< $difference->in_units('seconds') >> looks promising, but will
...
actually represents.  I don't know why the designer of
DateTime::Duration thought it would be a good idea to throw this
information away, but that's what it does.  And I don't know why the
designer of DateTime thought it would be a good idea to overload
the subtraction operator to make it easy to throw this information
away, but that's what it does.

Similarly if you wanted to know how many days there are between the
...
this is to calculate the 536 months by 30.4375, which is the average
number of days in a month, and add 23.  This is clumsy, and gets you
16337 days—which is close, but wrong.
...
DateTime::Moonpig introduces four main changes to the interface of
DateTime:
...
DateTime::Moonpig object yields the difference in seconds, as an
...
just divide by 3600.  Similarly, dividing this by 3600 gets the number
of hours difference.

Also, an integer number of seconds can be added to or subtracted from
...
it has bad interactions with C<set_time_zome>, which
C<DateTime::Moonpig> does I<not> disable.
