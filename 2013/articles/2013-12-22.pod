...
given repository, and the C<got tmux REPO> subcommand (contributed by Yannick
...
can use the C<got this> command (contributed by Mike Greb) while inside the
repository. If it is managed by C<got>, you'll get back the listing output for
the repo. If it is not managed by C<got>, you'll see a message to that effect.
