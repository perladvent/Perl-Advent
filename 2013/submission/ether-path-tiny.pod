...
toolbox that lets me forget about File::Spec, File::Copy, File::Path,
File::Temp and the rest -- Path::Tiny.  It's amazingly full-featured for a
module that can still rightfully claim the Tiny name.
...
files, automatically respecting the C<TMPDIR> environment variable?
    ...
    my $tempdir = Path::Tiny->tempdir('delivery_list__XXXXXX');
...
Or that you can use it to easily process your files?
    ...
    foreach my $city_dir (path(qw(master_list 2013 North_America))->children)
    {
        ...
        {
            # recipient will be the same as from path(qw(master_list 2013 North_America Vancouver ETHER))
...
You can even traverse directories recursively:
    ...
    while (my $path = $iter->())
    {
        ...
        process_giftlist($path) if $path->is_file;
    }
...
Path::Tiny also makes creating files and directories a breeze:
    ...
    my $year = '2013';  # or POSIX::strftime('%Y', localtime)
