...
The corresponding Perl code for this would be the following, more or less:
  ...
  my $formula = sub {
    return ($_[0]{number_of_tantrums} / 6)
     + ($_[0]{age} / 12 * (
        $_[0]{teeth_brushs_this_year}+flosses_this_year)
        )/2
  }
...
at all, but things like the value of the gift and number left in stock are
...
Eval::Closure lets us control exactly what variables the code we compile has
