...
"Mr Greentriffle", the Wise Old Elf interjected, "May I be of some assistance?"
...
example code was so simple we already know what scalar that is.  But, if we
...
takes a while to execute, but it can work out cumlative
...
has a loop), but obviously not what variable holds them (because none does):
...
You might be temped to search for a hash key.  This won't go as well:
