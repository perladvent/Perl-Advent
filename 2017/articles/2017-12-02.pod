...
were really simple, as was the code she'd hastely written to do so.
...
for the US flag, nor any for any of the other countries.

After much head scratching, and a relaxing glass of mulled wine, Sherry
discovered that not all glyphs in Unicode are represened by a single character.
Some glyphs are made up by combinging one or more characters:

    #!perl
    print "\N{REGIONAL INDICATOR SYMBOL LETTER U}";
    print "\N{REGIONAL INDICATOR SYMBOL LETTER S}";

On systems that don't understand how to render flags that simply would display
...
capible systems that would be turned into a United States flag emoji.

So, that was simple - sort of - but Sherry still needed some way to turn country
code that her subroutine was being passed into those corrisponding regional
indicator characters.  Of course, this being Perl, there was a module for that
on the CPAN:
