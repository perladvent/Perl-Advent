...
their battletested Perl code in Go.  What he really needed
...
Go is perfectly capible of producing a shared library.
...
We've got two new file, the header file (the C<merrychristmas.h>)
...
excellent Forien Function Interface libraries to access the shared
object instead.
...
we access it from Perl?
    ...
    # bind the exported "MerryChristmas" to the "MerryChristmas"
    # function in Perl space, and declare that it takes no args
    # and returns nothing
    $ffi->attach(MerryChristmas => []);
...
We can make this a whole lot clearer if we just use one
of 
    ...
    # then make use of it idomatically when we attach
...
the Go string structure it expects which looks somewhat
like this in C space:
...
There's several aproaches we can take to helping cross
the divide.  The simpliest solution is to define in
...
This allows us to create records easily from Perl:
