...
in case, say, at 6am on Christmas Morning a car containing
...
forcasting for up to a week in advance, per minute rainfall for the next
hour in easy to read graphical form, and a compelling timemachine
...
the local IP address that's been assignd to the computer we're
...
several JSON API services that will happlily do this for us.
...
of IP addresses to aproximate locations that we can use
...
use to access these databases without much work:
    ...
    # these are only aproximate, but good enough
